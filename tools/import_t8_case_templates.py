...
def build_catalog(registry_path: Path, batch_path: Path) -> dict[str, Any]:
    ...
    for item in batch.get("cases", []):
        ...
        if not re.fullmatch(r"t8c\d{3}-[a-z0-9-]+", template_id):
    ...
    return {
        ...
        "catalog_id": f"t8-curated-{batch.get('batch_id')}",
        "authority": batch.get("authority", "T8 精选案例（非官方）"),
...
def main() -> int:
    ...
    catalog = build_catalog(args.registry.resolve(), args.batch.resolve())
