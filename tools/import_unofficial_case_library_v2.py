...
EXPECTED_RECORD_COUNT = 113
EXPECTED_SELECTOR_COUNT = 98
EXPECTED_EVIDENCE_COUNT = 15
EXPECTED_PENDING_COUNT = 0
EXPECTED_COMMUNITY_SKILL_COUNT = 2
EXPECTED_TOTAL_SELECTOR_COUNT = 100
EXPECTED_CONTRACT = {
...
def build_catalog(
    ...
    if declared_counts != expected_counts or actual_counts != expected_counts:
        raise LibraryImportError(
            "Expected 113 records: 98 selectors, 15 evidence variants, and no pending cases"
        )
    ...
    if len(templates) != EXPECTED_TOTAL_SELECTOR_COUNT:
        raise LibraryImportError("Expected 100 total non-official selectors")
    return {
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Import the 113-record case handoff: 98 selectors, 15 evidence variants, "
