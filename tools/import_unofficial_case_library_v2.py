...
def build_catalog(
    ...
    if declared_counts != expected_counts or actual_counts != expected_counts:
        raise LibraryImportError(
            "Expected 87 records: 77 selectors, ten evidence variants, and no pending cases"
        )
    ...
    if len(templates) != EXPECTED_TOTAL_SELECTOR_COUNT:
        raise LibraryImportError("Expected 79 total non-official selectors")
    return {
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Import the 87-record case handoff: 77 selectors, ten evidence variants, "
