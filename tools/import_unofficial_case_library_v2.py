...
def build_catalog(
    ...
    if declared_counts != expected_counts or actual_counts != expected_counts:
        raise LibraryImportError(
            "Expected 66 records: 58 selectors, eight evidence variants, and no pending cases"
        )
    ...
    if len(templates) != EXPECTED_TOTAL_SELECTOR_COUNT:
        raise LibraryImportError("Expected 60 total non-official selectors")
    return {
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Import the 66-record case handoff: 58 selectors, eight evidence variants, "
