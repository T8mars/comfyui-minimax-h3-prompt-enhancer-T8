            ...
            [
                ...
                _suite_node(
                    ...
                    inputs=[
                        ...
                        _socket("api_key", "STRING", None),
                        _socket("provider_config", "T8_LLM_PROVIDER_CONFIG", 3),
                        _socket("film_project_state", "T8_FILM_PROJECT_STATE", 8),
                    ],
                    ...
                    widgets=["维修机器人护送植物抵达城市东侧温室", "MiniMax H3 + Seedance 2.0", 90, 15, "状态连续（推荐）", "balanced", "中文", 0, "fixed", "机器人划痕、植物叶片数量、东方移动方向保持一致", "严格阻止下游（推荐）"],
                    size=[580, 620],
                ),
                _suite_node(
                    4, "T8StoryboardPack", [1800, 20],
                    inputs=[
                        _socket("creative_brief", "T8_CREATIVE_BRIEF", 2),
                        _socket("reference_role_map", "T8_REFERENCE_ROLE_MAP", None),
                        _socket("performance_director_config", "T8_PERFORMANCE_DIRECTOR_CONFIG", None),
                        _socket("api_key", "STRING", None),
                        _socket("provider_config", "T8_LLM_PROVIDER_CONFIG", 4),
                        _socket("film_project_state", "T8_FILM_PROJECT_STATE", 9),
                        _socket("character_performance_bible", "T8_CHARACTER_PERFORMANCE_BIBLE", 12),
                    ],
                    ...
                    widgets=["为第一段设计可执行分镜", "MiniMax H3 + Seedance 2.0", 15, "5", "balanced", "中文", 0, "fixed", "严格阻止下游（推荐）"],
                    size=[580, 560],
                ),
                ...
                _suite_node(
                    8, "T8FilmProjectRouter", [520, 840],
                    inputs=[_socket("previous_state", "T8_FILM_PROJECT_STATE", None)],
                    outputs=[
                        ("film_project_state", "T8_FILM_PROJECT_STATE", [8, 9]),
                        ("active_stage_context", "STRING", None),
                        ("project_state_json", "STRING", None),
                        ("invalidation_report_json", "STRING", None),
                    ],
                    widgets=[
                        "停电城市护送", "修订（保留版本并标记影响）",
                        "08-prompt | 08 H3 / Seedance 提示词", "维修机器人护送一株植物前往温室。",
                        "screenplay-v03\ncharacter-v02", "01 02 03 04 05 06 07",
                        "05-screenplay | 05 剧本", "第三段行动方向发生修订",
                        "城市停电后只有应急机械线路可用", "机器人每次强行供电都会永久损失一组记忆",
                        "机器人不知道温室已经切换为备用门",
                        "机器人划痕、植物叶片数量、东方移动方向保持一致", "",
                    ],
                    size=[560, 650],
                ),
                _suite_node(
                    9, "T8CharacterPerformanceBible", [1140, 680], inputs=[],
                    outputs=[
                        ("character_performance_bible", "T8_CHARACTER_PERFORMANCE_BIBLE", [10]),
                        ("performance_contract_text", "STRING", None),
                        ("performance_bible_json", "STRING", None),
                    ],
                    widgets=[
                        "维修机器人", "在电量耗尽前把植物交给温室管理员",
                        "城市断电、路线改变；失败会让植物死亡", "绕行\n请求协助\n牺牲记忆供电",
                        "双臂始终托住花盆；上一段的机体迟滞继续累积",
                        "短促合成音；电量下降时断句变长",
                        "植物叶片枯萎时，机械式镇定被打破",
                        "先检查植物，再看路线标识；接收新信息后才行动",
                    ],
                    size=[580, 500],
                ),
                _suite_node(
                    10, "T8CharacterPerformanceBible", [1140, 1240], inputs=[],
                    outputs=[
                        ("character_performance_bible", "T8_CHARACTER_PERFORMANCE_BIBLE", [11]),
                        ("performance_contract_text", "STRING", None),
                        ("performance_bible_json", "STRING", None),
                    ],
                    widgets=[
                        "温室管理员", "确认植物仍可救治并打开备用门",
                        "机器人正在失去记忆；判断错误会错过救治窗口", "观察叶片\n核对路线\n打开备用门",
                        "左手持续按住应急门开关，直到机器人进入",
                        "平稳短句；只在说话时使用",
                        "机器人忘记植物名称时，专业镇定短暂破裂",
                        "先看植物，再看机器人状态灯；确认后才开门",
                    ],
                    size=[580, 500],
                ),
                _suite_node(
                    11, "T8CharacterPerformanceBibleStack", [1800, 760],
                    inputs=[
                        _socket("character_performance_bibles.character_performance_bible_0", "T8_CHARACTER_PERFORMANCE_BIBLE", 10, label="character_performance_bible_0"),
                        _socket("character_performance_bibles.character_performance_bible_1", "T8_CHARACTER_PERFORMANCE_BIBLE", 11, label="character_performance_bible_1"),
                    ],
                    outputs=[
                        ("character_performance_bible_set", "T8_CHARACTER_PERFORMANCE_BIBLE", [12]),
                        ("performance_contract_text", "STRING", None),
                        ("performance_bible_set_json", "STRING", None),
                    ],
                    widgets=[], size=[540, 260],
                ),
            ],
            [
                [1, 2, 0, 3, 0, "T8_CREATIVE_BRIEF"], [2, 2, 0, 4, 0, "T8_CREATIVE_BRIEF"],
                [3, 1, 0, 3, 2, "T8_LLM_PROVIDER_CONFIG"], [4, 1, 0, 4, 4, "T8_LLM_PROVIDER_CONFIG"],
                [5, 3, 1, 5, 0, "STRING"], [6, 3, 2, 6, 0, "STRING"], [7, 4, 0, 7, 0, "STRING"],
                [8, 8, 0, 3, 3, "T8_FILM_PROJECT_STATE"],
                [9, 8, 0, 4, 5, "T8_FILM_PROJECT_STATE"],
                [10, 9, 0, 11, 0, "T8_CHARACTER_PERFORMANCE_BIBLE"],
                [11, 10, 0, 11, 1, "T8_CHARACTER_PERFORMANCE_BIBLE"],
                [12, 11, 0, 4, 6, "T8_CHARACTER_PERFORMANCE_BIBLE"],
            ],
        ),
...
def check() -> None:
    ...
    expected_inputs = {
        ...
        "T8LongFormPlanner": ["creative_brief", "api_key", "provider_config", "film_project_state"],
        "T8StoryboardPack": [
            "creative_brief", "reference_role_map", "performance_director_config", "api_key", "provider_config",
            "film_project_state", "character_performance_bible",
        ],
        "T8FilmProjectRouter": ["previous_state"],
        "T8CharacterPerformanceBible": [],
        "T8CharacterPerformanceBibleStack": [
            "character_performance_bibles.character_performance_bible_0",
            "character_performance_bibles.character_performance_bible_1",
        ],
    ...
    required = {
        ...
        "T8FilmProjectRouter", "T8CharacterPerformanceBible", "T8CharacterPerformanceBibleStack",
