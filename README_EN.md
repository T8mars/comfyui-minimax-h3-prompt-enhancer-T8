...
For local GGUF inference, the selected output language is repeated as a final authoritative instruction after all embedded Skills and examples. This applies to H3, Seedance 2.0, and Music 3 Structured Caption prose. If the result is an obvious dominant-language mismatch, the node performs one low-temperature correction while the same local model remains loaded; conforming output adds no extra call. Required protocol fields, timestamps, reference labels, and exact user dialogue, lyrics, brand/UI copy, and visible text remain unchanged. H3's strict all-English official profile still intentionally overrides the Chinese selector.
