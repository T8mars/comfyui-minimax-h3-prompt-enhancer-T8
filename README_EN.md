...
- Includes 554 released T8 case records consolidated into 240 stable selectors, including 314 evidence variants, plus two independent community Skills. All 242 non-official selectors include Chinese names, concise suggested inputs, structural anchors, and lightweight GIF previews.
...
The Registry package contains the node runtime code, pinned official Skills, the non-official case library, eight official GIFs, and a small preview-channel manifest. The 556 T8 GIFs are not embedded in the Registry ZIP. When a user opens a T8 case, the node downloads only its small release shard and caches it under `ComfyUI/user/t8_prompt_enhancer/preview_assets`. The package still excludes API keys, tests, source delivery batches, the standalone `llama-server` runtime, install/download scripts, and GGUF weights. Local GGUF mode in a Manager install uses `llama-cpp-python` from the active ComfyUI Python environment. Use the recommended full GitHub install when you need the pinned `llama-server`, PATH fallback, environment defaults, or the explicit credential connection probe.

All 564 official/T8 previews remain available. The main repository retains the eight official GIFs and the previous 377 T8 GIFs as an offline baseline; all 556 T8 previews, including new additions, are updated through the versioned [`comfyui-minimax-h3-prompt-enhancer-T8-assets`](https://github.com/T8mars/comfyui-minimax-h3-prompt-enhancer-T8-assets) release channel. Shards, individual GIFs, catalog identities, and source hashes are verified before cache installation. Offline or failed preview downloads never stop prompt enhancement.
