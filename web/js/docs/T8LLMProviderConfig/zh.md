...
“共享 LLM 渠道配置（可选）”是专用配置类型，不是 `STRING`。在画布中搜索 `T8 LLM 共享渠道配置`，把其“渠道配置”输出连接到核心节点的绿色同名输入。选择 `Local Qwen` 后不需要 API Key；主模型递归扫描 `ComfyUI/models/LLM`，H3/Seedance 的视觉分析可用 AUTO 匹配 mmproj，Music 3 只加载文字模型。
