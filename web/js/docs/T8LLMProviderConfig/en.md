...
The optional shared-provider socket is a dedicated configuration type, not a `STRING`. Add `T8 LLM Provider Config` and connect its output to the green shared-provider input. `Local Qwen` requires no API key; H3 and Seedance use the configured mmproj for sampled visual frames, while Music 3 loads the text model only.
