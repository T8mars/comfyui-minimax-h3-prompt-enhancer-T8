...
- `本地单次生成 Token（含思考）` 同时支付模型的思考过程和最终正文，旧版 8192 是项目的保守限制，并非 Qwen/llama.cpp 硬上限。新建节点及仓库示例默认 `16384`，可手动设置 `256–61440`；已有工作流保留其原先保存值，需用户按需改大。输出预算必须至少比 `local_context_size` 小 1024 Token，并且还要为输入提示词和视觉部件留空间；例如设置超过 `31743` 时，应先把上下文从默认 `32768` 提高到 `65536`。更大上下文会增加内存/显存占用。
