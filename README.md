...
- 两个节点共享独立的 `非官方模板（案例 / 社区 Skill）` 列表：573 条已发布案例事实归并为 240 个稳定案例 selector（含 333 个同机制证据变体），另有 2 个独立用户贡献社区 Skill，共 242 个非官方下拉项；全部提供中文名称、用途、简约推荐输入、2–5 个结构锚点和轻量 GIF 预览。
- 支持中文 / English 输出。
...
- 583 个官方/T8 GIF 全部可用；主仓库保留 8 个官方 GIF 和既有 377 个 T8 GIF，新加入的 T8 预览不再继续扩充主包。全部 575 个 T8 GIF 都可由独立版本化资源通道按需校验、下载和更新，避免 GitHub/Registry 包随案例增长持续膨胀。
...
GIF 只供浏览器中的人类界面预览，绝不会连接到首帧、尾帧、参考图片、参考视频，也不会发送给 LLM。8 个官方预设 GIF 继续内置；主仓库保留此前的 377 个 T8 轻量 GIF 作为离线基线，当前全部 575 个 T8 案例/社区 Skill 预览统一通过独立的 [`comfyui-minimax-h3-prompt-enhancer-T8-assets`](https://github.com/T8mars/comfyui-minimax-h3-prompt-enhancer-T8-assets) Release 动态更新。节点会校验安装目录对应的案例全集、来源 SHA-256、分片 SHA-256、每个 GIF 的大小/哈希/文件头和安全解包路径；不兼容或损坏的资源不会显示，也不会影响提示词增强。
...
| `case_template` | `无 / 242 个非官方模板`：240 个 T8 案例 selector + 2 个独立社区 Skill；显示中文名称，工作流保存稳定 ID，H3 与 Seedance 2.0 使用独立原生适配规则；H3 中与官方场景 Skill 同选时优先生效 |
