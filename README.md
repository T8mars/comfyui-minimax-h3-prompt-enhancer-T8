...
- 两个节点提供独立的 `T8 精选案例模板（非官方）` 列表，当前 17 个已发布案例只携带可复用 Creative DNA 与防复制规则，不携带源视频、路径、URL 或成品提示词。
- 支持中文 / English 输出。
...
该列表与 MiniMax 官方创意预设完全分开。当前收录 17 个已发布模板；两个节点使用同一稳定模板 ID，但 H3 版本会折叠到 H3 字段、`[Shot N]` 时间线和声音合同；Seedance 2.0 版本会使用任务意图、自然段或 `镜头N` 顺序与素材引用语法，不输出 H3 字段或绝对 H3 时间码。
...
| `case_template` | `无 / 17 个 T8 精选案例模板（非官方）`；H3 与 Seedance 2.0 使用独立适配规则 |
