...
- 两个节点提供独立的 `T8 精选案例模板（非官方）` 列表，当前 26 个已发布案例只携带可复用 Creative DNA 与防复制规则，不携带源视频、路径、URL 或成品提示词。
- 支持中文 / English 输出。
...
该列表与 MiniMax 官方创意预设完全分开。当前收录 26 个已发布模板；两个节点使用同一稳定模板 ID，但 H3 版本会折叠到 H3 字段、`[Shot N]` 时间线和声音合同；Seedance 2.0 版本会使用任务意图、自然段或 `镜头N` 顺序与素材引用语法，不输出 H3 字段或绝对 H3 时间码。
...
| `泰然连锁失控` | 正式仪式中由单一物理负担触发递增连锁后果，以泰然表演和可见失效完成反转 |
| `分层档案激活` | 固定人物与模块网格，按身份、诊断、能力、工具和状态层级逐步激活档案 |
| `创作越界光潮` | 被创作的小型标记获得自主性并越过实体边界，扩大为能改变环境的可见后果 |
| `材质进度时钟` | 围绕同一工艺物件，用不可逆且只增不减的材料变量充当制作进度时钟 |
| `四阶角色亮相` | 由标志细节、局部轮廓、面部、单一动作到完整定格逐级释放稳定角色身份 |
| `机械因果伴行` | 机械装置达到可见阈值后让负载起浮，以连续视角证明稳定的前后编队关系 |
| `平面几何重构` | 围绕单一几何母题拆解重构平面布局，经高密状态与对照静段收束为原创图形系统 |
| `声画错位递进` | 让同一固定提前量的声画错位从环境线索逐级升级到主体关键机构，并以未揭示的新声源收束 |
| `固定角色卡反复校验` | 让同一固定角色卡在姿态、动作、步态和面部证据之间反复出现，最后由实景主体完成定格 |
| `双系统汇聚证明` | 先分别建立两套相反但连续的主体与材质规则，再用一次不可逆接触形成可见的统一结果 |
| `目标障碍交替走廊` | 锁定远端目标，以前后交替的障碍和一次可控近失误持续证明主体的推进与保护能力 |
| `静密决三态终结` | 用安静建立、密集交换、单次决定性介入和延迟物理结果构成清楚的终结节奏 |
| `路线递减阈值逃生` | 主体始终朝固定终点前进，已通过路线单调消失，并在最后连接失效前完整跨过安全阈值 |
| `程序失配嵌套反转` | 让不合尺度的主体认真执行熟悉程序，检测先揭示不同功能载荷，再由实物行动完成双重证明 |
| `基底动作串联技能` | 用持续返回的基础动作连接递增技能，每种机位只承担一个运动证明任务，峰值后回到原循环 |
| `静态尺度主动介入` | 开场用静态标准物校准微缩尺度，内部日常稳定后由不同外部执行物单次介入并改变局部状态 |

2026-08-10-02 批次共完成 10 个双模型案例，其中 9 个作为新模板发布；另一条 MiniMax H3 平面几何案例用于验证既有 `平面几何重构` 机制，不重复增加下拉项。
...
python tools/import_t8_case_templates.py \
  ...
  --batch case_templates/source_batches/2026-08-10-02.json \
...
| `case_template` | `无 / 26 个 T8 精选案例模板（非官方）`；H3 与 Seedance 2.0 使用独立适配规则 |
